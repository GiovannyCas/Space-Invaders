...
namespace space_invaders {
    ...
    Invader_Manager::Invader_Manager(World& world)
        : m_stepGap(sf::seconds(0.5f))
        , m_world(world)
        , m_invaderRenderer(12, 8, Invader::WIDTH, Invader::HEIGHT,
            Core::Resource_Holder::get().textures.get("si/invaders"))
    {
        ...
        // load sounds
        for (int i = 0; i < 4; i++) {
            m_stepSounds[i].setBuffer(Core::Resource_Holder::get().soundBuffers.get(
                "si/fastinvader" + std::to_string(i + 1)));
        }
        m_invaderKilledSound.setBuffer(
            Core::Resource_Holder::get().soundBuffers.get("si/invaderkilled"));
    }

    void Invader_Manager::tryStepInvaders()
    {
    ...
    void Invader_Manager::drawInvaders(sf::RenderTarget& target)
    {
        for (auto& invader : m_invaders) {
            if (!invader.isAlive())
                continue;
    ...
    CollisionResult
        Invader_Manager::tryCollideWithProjectiles(std::vector<Projectile>& projectiles)
    {
    ...
    sf::Vector2f Invader_Manager::getRandomLowestInvaderPoint(Core::Random<>& random)
    {
        if (m_aliveInvaders == 0)
            return { -1, -1 };
    ...
    int Invader_Manager::getAliveInvadersCount() const
    {
        return m_aliveInvaders;
    }

    // Adds invaders to scene 1 at a time
    void Invader_Manager::initAddInvader()
    {
    ...
    bool Invader_Manager::areInvadersAlive() const
    {
        return m_hasAllInvadersBeenAdded;
    }

    void Invader_Manager::updateStepDelay()
    {
        m_stepGap = sf::seconds((float)m_aliveInvaders / 90.0f);
    }

    bool Invader_Manager::testInvaderPosition(const Invader& invader) const
    {
