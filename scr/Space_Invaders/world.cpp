...
namespace space_invaders {
	...
	World::World()
		: m_projectileRenderer(
			...
			Core::Resource_Holder::get().textures.get("si/projectile"))
		, m_invaders(*this)
		, m_alien(m_rng)
	{
		m_explodeShape.setSize({ 52, 28 });
		m_explodeShape.setTexture(
			&Core::Resource_Holder::get().textures.get("si/explosion"));
		...
		for (int i = 0; i < 4; i++) {
		 ...
		 m_playerShoot.setBuffer(
			 Core::Resource_Holder::get().soundBuffers.get("si/shoot"));
	...
	CollisionResult World::getCollisionResult(float dt)
	{
		...
		for (auto& projectile : m_projectiles) {
			...
			if (m_alien.tryCollideWith(projectile)) {
				result.second.emplace_back(projectile.getPosition());
				result.first += 200;
			}
